...
T_C = 2.0 / log(1.0 + sqrt(2.0))  # critical temperature
Beta_C = 1.0 / T_C  # inverse critical temperature
...
def exact_energy(temp: float) -> float:
    beta = 1.0 / temp
...
def exact_specific_heat(temp: float) -> float:
    beta = 1.0 / temp
    cc, ss, tt = cosh(2.0 * beta), sinh(2.0 * beta), tanh(2.0 * beta)
    k = 2.0 * ss / cc**2
    kk = ellipk(k * k)
...
def exact_magnetization(temp: float) -> float:
    beta = 1.0 / temp
    if beta <= Beta_C:
        return 0
    else:
